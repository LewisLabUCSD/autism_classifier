...
dataMain=.myDataArrangerFn(inputPath="~/Documents/archive/papers/classificationPaper/HT_as_test_05142019/ageBalanced_decov_genes/mainDataset_HT12_WG6_deconv_genes/classificationSet_subject_scores/",inputROCthr=0.7,inputROCPower=2,colorset=c("black","red"))


dataTest=.myDataArrangerFn(inputPath="~/Documents/archive/papers/classificationPaper/HT_as_test_05142019/ageBalanced_decov_genes/validation/testDataset_HT12/",inputROCthr=0.7,inputROCPower=2,colorset=c("black","red"))


dataLong=.myDataArrangerFn(inputPath="~/Documents/archive/papers/classificationPaper/HT_as_test_05142019/ageBalanced_decov_genes_wF_v2/validation/Longitudinaltest/",inputROCthr=0.7,inputROCPower=2,colorset=c("black","red"))#,wCompleteDS=T)




dataLD=.myDataArrangerFn(inputPath="~/Documents/archive/papers/classificationPaper/HT_as_test_05142019/ageBalanced_decov_genes_wF_v2/validation/LDtestDataset_HT12/",inputROCthr=0.7,inputROCPower=2,colorset=c("black","green"))#,wCompleteDS=T)

#save(dataLD,dataMain,dataLong,dataTest,file="~/Desktop/myData/classification_age_balanced05162019_wF_v2.rda")

save(dataLD,dataMain,dataLong,dataTest,file="~/Desktop/classification_age_balanced05162019.rda")
...
```{r}
rm(list=ls())
load("~/Documents/archive/UCSD_data/myData/classification_age_balanced05162019.rda")
...
plot(Mainvis$ROCdata,xlim=c(1,0),ylim=c(0,1),main="ROC main")
Testvis=.myClassificationEvaluationFn(dataTest$data$concensus$weightedEstimate,dataTest$data$concensus$real)
plot(Testvis$ROCdata,xlim=c(1,0),ylim=c(0,1),main="ROC test")
...
ggplot(DEmain$pvalDist,aes(pval,color=class))+geom_density()+theme_bw()+theme(axis.text = element_text(color="black"),panel.grid = element_blank())+ggtitle("Evaluation dataset")
...
ggplot(DEtest$pvalDist,aes(pval,color=class))+geom_density()+theme_bw()+theme(axis.text = element_text(color="black"),panel.grid = element_blank())+ggtitle("Test dataset")
rm(expTest,testData)
