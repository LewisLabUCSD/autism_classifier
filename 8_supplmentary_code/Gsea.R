...
gene_sets_dir <- "/Volumes/Work/Vahid_work/classification_newcode/autism_classifier/8_supplmentary_code/"
...
AnnotDEG_byGSEA <- function(DEGTable = NULL, 
  ...
  for (c in GSEA_set) {
    gmtfile_name <-
      paste0(gene_sets_dir,"msigdb_v5.1_GMTs/", c, ".all.v5.1.symbols.gmt")
...
ExportExcel <- function(out, exportExcel){
  ...
  if(length(out) > 0){
    ...
    print("preExcel writing")
    write.xlsx(out,file=exportExcel)
    #write.csv(out,file=exportExcel,quote=F,row.names=F,na = 1)
    print("postExcel writing")
...
ExportPDF <- function(y,fp,wth_factor,ShowTop, TopN){
  ...
  pdf(file=fp, width=20, height=30*wth_factor,onefile = TRUE)
  p <- ggplot(y.m, aes(x=names, y=NES)) +
       geom_bar(stat = 'identity', aes(fill = NES))  +
       scale_fill_gradient2(low = "red", high = "blue", mid = "white", midpoint = 0, limits=c(-5,5)) +
