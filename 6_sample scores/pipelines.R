...
.myPackageInstaller=function(){
  ...
  for(i in 1:length(bioconductorPackages)){
    if((bioconductorPackages[i] %in% rownames(installed.packages())) == FALSE) {BiocManager::install(bioconductorPackages[i])}
  }
...
.myWGCNAwrapperFn=function(inputData,labels,testInputData,testLabels,expClassName,tmpMethod){
  ...
  if(length(resWgcna)>0)
  {
    ...
    row.names(tstInputData)=colnames(testInputData)
    features=resWgcna[["eigenGenes"]]
    row.names(features)=colnames(inputData)
    resWgcna=list(list(labels=labels,features=features,testInputData=tstInputData,testLabels=testLabels,expClassName=expClassName,method="wgcna"))
  }
...
.myCPPLSwrapperFn=function(inputData,labels,testInputData,testLabels,expClassName,tmpMethod=''){
  ...
  if("results" %in% names(resCppls)){
    ...
    resCppls=list(labels=labels,features=t(.matrixExtraction(inputData)),testInputData=t(.matrixExtraction(testInputData)),testLabels=testLabels,method="")
  }
  return(resCppls)
}
