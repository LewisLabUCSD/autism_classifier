.myVarGeneFilterationFn=function(inputExpSet,labels,testInputData,outputsize=NULL){
  vars=apply(.matrixExtraction(inputExpSet),1,var)
  if(is.null(outputsize)){
    thr=summary(vars)[3]
    inputExpSet=inputExpSet[vars>thr,]
    testInputData=testInputData[row.names(inputExpSet),]
  }else{
    if(outputsize<1){
      outputsize=round(outputsize*dim(inputExpSet)[1]) 
    }
    selIds=names(tail(sort(vars),outputsize))
    inputExpSet=inputExpSet[selIds,]
    testInputData=testInputData[selIds,]
  }
  return(list(inputExpSet=inputExpSet,testInputData=testInputData))
}

.myCovGeneFilterationFn=function(inputExpSet,labels,testInputData,outputsize=NULL){
  sds=apply(.matrixExtraction(inputExpSet),1,sd)
  means=rowMeans(.matrixExtraction(inputExpSet))
  covs=sds/means
  
  if(is.null(outputsize)){
    thr=summary(covs)[3]
    inputExpSet=inputExpSet[covs>thr,]
    testInputData=testInputData[row.names(inputExpSet),]
  }else{
    if(outputsize<1){
      outputsize=round(outputsize*dim(inputExpSet)[1]) 
    }
    selIds=names(tail(sort(covs),outputsize))
    inputExpSet=inputExpSet[selIds,]
    testInputData=testInputData[selIds,]
  }
  return(list(inputExpSet=inputExpSet,testInputData=testInputData))
...
.myRNAseqLowFilter=function(inputExpSet,labels,minCount=3,minPercentage=0.7){
  thr=minCount
  labels=as.factor(labels)
  sum=0
  for(i in 1:length(levels(labels)))
  {
    x=.matrixExtraction(inputExpSet[,labels==levels(labels)[i]])<thr
    x=apply(x,1,sum)
    if(length(sum)==1&&sum==0)
      sum=!(x>0.3*dim(inputExpSet[,labels==levels(labels)[i]])[2])
    else
      sum=sum+(!(x>0.3*dim(inputExpSet[,labels==levels(labels)[i]])[2]))
  }
  
  inputExpSet=inputExpSet[which(sum>0),]
  return(inputExpSet)
}

.myRNAseqHighFilter=function(inputExpSet,count=20){
  thr=count
  rHE=data.frame(test=rep("a",thr),stringsAsFactors = F)
  for(i in 1:dim(data)[2]){
    tst=sort((.matrixExtraction(inputExpSet)[,i]))
    tst=data.frame(matrix(tail(names(tst),thr),ncol=1),stringsAsFactors = F)
    rHE=cbind(rHE,tst)
  }
  rHE=rHE[,-1]
  ...
  tst=as.vector(tst)
  tst=unique(tst)
  print(paste0("Number of removed genes: ",length(tst)))
  inputExpSet=inputExpSet[-which(rownames(inputExpSet) %in% tst),]
  return(inputExpSet)
}

.myGRNFilterationFn=function(inputExpSet,inputLabels,inputExpClassName,outputsize=NULL,method=NULL){
  require(minet,quietly=T)
  require(plyr,quietly=T)
  vars=apply(.matrixExtraction(inputExpSet),1,var)
  if(length(which(vars<0.01))>0)
    inputExpSet=inputExpSet[vars>summary(vars)[2],]
  
  #print(paste0("constructing the GRN, ",dim(inputExpSet)[1]," genes"))
 
  mim= build.mim(t(.matrixExtraction(inputExpSet[,inputLabels==inputExpClassName])))
  Zscore=clr( mim, skipDiagonal=1 )
  rm(mim)
  
  mimCntl= build.mim(t(.matrixExtraction(inputExpSet)))
  ZscoreCntl=clr( mimCntl, skipDiagonal=1 )
  rm(mimCntl)
  
  indx1=which(Zscore>2.5)
  indx2kt=lower.tri(Zscore,diag = F)
  indx2=seq(1,length(indx2kt))[indx2kt]
  thrZscore=Zscore[intersect(indx1,indx2)]
  thrZscore=sort(thrZscore,decreasing = F)
  thrPval=2.0*pnorm(-1*thrZscore)
  thrFDR=.myFDRfn(thrPval,(dim(Zscore)[1]*(dim(Zscore)[1]-1)/2))
  slZscoreThr=thrZscore[thrFDR<0.1]
  slZscoreThr=slZscoreThr[1]
  Zscore[!indx2kt]=0
  df=as.data.frame(which(Zscore>slZscoreThr,arr.ind = T))
  colnames(df)=c("Fnode","Snode")
  df$Zscore=Zscore[cbind(df$Fnode,df$Snode)]
  df$Fnode=row.names(inputExpSet)[df$Fnode]
  df$Snode=row.names(inputExpSet)[df$Snode]
  rm(Zscore)
  zscoredf=data.frame(Zscore=thrZscore,qvalue=thrFDR,Pval=thrPval)
  
  
  indx1=which(ZscoreCntl>2.5)
  indx2kt=lower.tri(ZscoreCntl,diag = F)
  indx2=seq(1,length(indx2kt))[indx2kt]
  thrZscoreCntl=ZscoreCntl[intersect(indx1,indx2)]
  thrZscoreCntl=sort(thrZscoreCntl,decreasing = F)
  thrPvalCntl=2.0*pnorm(-1*thrZscoreCntl)
  thrFDRCntl=.myFDRfn(thrPvalCntl,(dim(ZscoreCntl)[1]*(dim(ZscoreCntl)[1]-1)/2))
  slZscoreThrCntl=thrZscoreCntl[thrFDRCntl<0.1]
  slZscoreThrCntl=slZscoreThrCntl[1]
  ZscoreCntl[!indx2kt]=0
  dfCntl=as.data.frame(which(ZscoreCntl>slZscoreThrCntl,arr.ind = T))
  colnames(dfCntl)=c("Fnode","Snode")
  dfCntl$ZscoreCntl=ZscoreCntl[cbind(dfCntl$Fnode,dfCntl$Snode)]
  dfCntl$Fnode=row.names(inputExpSet)[dfCntl$Fnode]
  dfCntl$Snode=row.names(inputExpSet)[dfCntl$Snode]
  rm(ZscoreCntl)
  zscoredfCntl=data.frame(Zscore=thrZscoreCntl,qvalue=thrFDRCntl,Pval=thrPvalCntl)
  
  
  finalRes=list(grn1=list(),grn2=list(),grn3=list())
  if(nrow(df)>0){
  
  df=plyr::join(df, zscoredf, by='Zscore', type='left', match='all')
  df=df[!is.na(df$qvalue),]
  df=df[which(as.numeric(as.character(df$qvalue))<0.1),]
  
 
  
  rm(thrPval,thrFDR,thrZscore,slZscoreThr,zscoredf)
  if(is.null(method)){
    #print("applying the first GRN method")
    grn1=.myGRNFilterationFn1(df,outputsize)
    grn1$Gene=as.character(grn1$Gene)
    #print("applying the second GRN method")
    grn2=.myGRNFilterationFn2(df,outputsize)
    grn2$Gene=as.character(grn2$Gene)
    #print("applying the third GRN method")
    grn3=.myGRNFilterationFn3(df,outputsize)
    grn3$Gene=as.character(grn3$Gene)
    #print("done.")
    finalRes=list(grn1=grn1,grn2=grn2,grn3=grn3)
  } else {
    if(method=="grn1"){
      #print("applying the first GRN method")
      grn1=.myGRNFilterationFn1(df,outputsize)
      grn1$Gene=as.character(grn1$Gene)
      finalRes=list(grn1=grn1)
    } else {
      if(method=="grn3"){
        #print("applying the third GRN method")
        grn3=.myGRNFilterationFn3(df,outputsize)
        grn3$Gene=as.character(grn3$Gene)
        finalRes=list(grn3=grn3)
      } else {
        if(method=="grn2"){
          #print("applying the third GRN method")
          grn2=.myGRNFilterationFn2(df,outputsize)
          grn2$Gene=as.character(grn2$Gene)
          finalRes=list(grn2=grn2)
        }
      }
    }
  }
  ...
  if(nrow(dfCntl)>0){
    df=dfCntl
    zscoredf=zscoredfCntl
    thrPval=thrPvalCntl
    thrZscore=thrZscoreCntl
    thrFDR=thrFDRCntl
    slZscoreThr=slZscoreThrCntl
    colnames(df)=c('Fnode','Snode','Zscore')
    ...
    if(is.null(method)){
      #print("applying the first GRN method")
      grn1Cntl=.myGRNFilterationFn1(df,outputsize)
      grn1Cntl$Gene=as.character(grn1Cntl$Gene)
      #print("applying the second GRN method")
      grn2Cntl=.myGRNFilterationFn2(df,outputsize)
      grn2Cntl$Gene=as.character(grn2Cntl$Gene)
      #print("applying the third GRN method")
      grn3Cntl=.myGRNFilterationFn3(df,outputsize)
      grn3Cntl$Gene=as.character(grn3Cntl$Gene)
      #print("done.")
      finalRes=c(finalRes,list(grn1Cntl=grn1Cntl,grn2Cntl=grn2Cntl,grn3Cntl=grn3Cntl))
    } else {
      if(method=="grn1"){
        #print("applying the first GRN method")
        grn1=.myGRNFilterationFn1(df,outputsize)
        grn1$Gene=as.character(grn1$Gene)
        finalRes=c(finalRes,list(grn1Cntl=grn1))
      } else {
        if(method=="grn3"){
          #print("applying the third GRN method")
          grn3=.myGRNFilterationFn3(df,outputsize)
          grn3$Gene=as.character(grn3$Gene)
          finalRes=c(finalRes,list(grn3Cntl=grn3))
        } else {
          if(method=="grn2"){
            #print("applying the third GRN method")
            grn2=.myGRNFilterationFn2(df,outputsize)
            grn2$Gene=as.character(grn2$Gene)
            finalRes=c(finalRes,list(grn2Cntl=grn2))
          }
        }
      }
    }
...
.myGRNFilterationFn1=function(df,outputsize=NULL){
  if(is.null(outputsize)){
  ...
  selGenes=unique(c(as.character(df[,"Fnode"]),as.character(df[,"Snode"])))
  
  
  return(list(Gene=selGenes,net=df))
}

.myGRNFilterationFn2=function(df,outputsize=NULL){
  
  df=df[df$qvalue<0.05,]
  res="c"
  count=0
  #if(dim(df)[1]>5000){
    initialSlGenes=table(c(df[,"Fnode"],df[,"Snode"]))
    initialSlGenes=sort(initialSlGenes,decreasing = T)
    initialSlGenes=initialSlGenes[1:round(length(initialSlGenes)/10)]
    res=names(initialSlGenes)
    {
      df=df[!(df$Fnode %in% res),]
      df=df[!(df$Snode %in% res),]
    }
  #}
  
  #combinedName=paste0(".",df$Fnode,".",df$Snode,".")
  #df$combinedName=combinedName
  
  #while(nrow(df)>0){
   # tryCatch({
    #  selGenes=table(c(df$Fnode,df$Snode))
    #  selGenes=sort(selGenes,decreasing = T)
      
    #  tmpGene=names(selGenes)[1:max(length(selGenes)/100,1)]
      
     #   df=df[! (df$Fnode %in% tmpGene),]
      #  df=df[! (df$Snode %in% tmpGene),]
      #  print(nrow(df))
      #res=c(res,tmpGene)
  ...
  res=data.frame(Gene=res[-1])
  if(!is.null(outputsize)){
    if(dim(res)[1]>outputsize){
      res=res[1:outputsize,]
    }
  }
  return(res)
}

.myGRNFilterationFn3=function(df,outputsize=NULL){
  df=df[df$qvalue<0.05,]
  res="c"
  count=0
  while(length(df[,1])>0){
    tryCatch({
      selGenes=table(c(df[,"Fnode"],df[,"Snode"]))
      slGenes=as.numeric(selGenes)
      names(slGenes)=row.names(selGenes)
      slGenes=sort(slGenes,decreasing = T)
      tmpGene=names(slGenes)[1]
      tobeRem=tmpGene
      tobeRem=c(tobeRem,df[(df$Fnode %in% tmpGene),'Snode'])
      tobeRem=c(tobeRem,df[(df$Snode %in% tmpGene),'Fnode'])
      df=df[!(df$Fnode %in% tobeRem),]
      df=df[!(df$Snode %in% tobeRem),]
      res=c(res,tmpGene)
      count=c(count,slGenes[1])
    }, warning = function(w) {
  ...
  res=data.frame(Gene=res[-1],hubness=count[-1])
  if(!is.null(outputsize)){
    if(dim(res)[1]>outputsize){
      res=res[1:outputsize,]
    }
...
.myZscoreFilterationFn=function(inputExpSet,labels,outputsize=NULL){
  require(minet,quietly=T) 
  vars=apply(.matrixExtraction(inputExpSet),1,var)
  if(length(which(vars<0.01))>0)
    inputExpSet=inputExpSet[vars>summary(vars)[2],]
  ...
  mim= build.mim(t(rbind(labels,.matrixExtraction(inputExpSet))))
  Zscore=clr( mim, skipDiagonal=1 )
  selGenes=Zscore[1,]
  selGenes=selGenes[order(selGenes,decreasing = T)]
  if(is.null(outputsize)){
    selGenes=names(selGenes)[selGenes>summary(selGenes)[5]]
  } else {
    if(outputsize<1){
      outputsize=round(outputsize*dim(inputExpSet)[1])
    }
    selGenes=names(selGenes)[1:outputsize,]
  }
  
  return(selGenes)
}

.mySISFilterationFn=function(inputExpSet,labels){
  require(SIS,quietly=T)
  labels=as.factor(as.character(labels))
  log <- capture.output({
    model21=SIS(t(.matrixExtraction(inputExpSet)), as.numeric(labels)-1, family='binomial',tune="cv");
    })
  
  return(row.names(inputExpSet)[model21$ix])
}

.mySelectVFiltrationFn=function(inputExpSet,labels){
  print("input is supposed to be log transformed data (approach developed on microarray data)")
  require(HiDimDA,quietly=T)
  
  ResCommonVar <- array(dim=4)
  names(ResCommonVar) <- c("ExpHC","HC","Fdr","Fair")
  expHCcv=SelectV(t(.matrixExtraction(inputExpSet)),as.factor(labels),Selmethod = 'ExpHC')
  HCcv=SelectV(t(.matrixExtraction(inputExpSet)),as.factor(labels),Selmethod="HC")
  FDRcv=SelectV(t(.matrixExtraction(inputExpSet)),as.factor(labels),Selmethod="Fdr")
  FairCv=SelectV(t(.matrixExtraction(inputExpSet)),as.factor(labels),Selmethod="Fair")
  ResCommonVar[1] <- list(list(genes=row.names(inputExpSet)[expHCcv$vkptInd], size=expHCcv$nvkpt))
  ResCommonVar[2] <- list(list(genes=row.names(inputExpSet)[HCcv$vkptInd], size=HCcv$nvkpt))
  ResCommonVar[3] <- list(list(genes=row.names(inputExpSet)[FDRcv$vkptInd], size=FDRcv$nvkpt))
  ResCommonVar[4] <- list(list(genes=row.names(inputExpSet)[FairCv$vkptInd], size=FairCv$nvkpt))
  
  ResNotCommonVar <- array(dim=4)
  names(ResNotCommonVar) <- c("ExpHC","HC","Fdr","Fair")
  expHCncv=SelectV(t(.matrixExtraction(inputExpSet)),as.factor(labels),Selmethod = 'ExpHC',comvar=F)
  HCncv=SelectV(t(.matrixExtraction(inputExpSet)),as.factor(labels),Selmethod="HC",comvar=F)
  FDRncv=SelectV(t(.matrixExtraction(inputExpSet)),as.factor(labels),Selmethod="Fdr",comvar=F)
  FairnCv=SelectV(t(.matrixExtraction(inputExpSet)),as.factor(labels),Selmethod="Fair",comvar=F)
  ResNotCommonVar[1] <- list(list(genes=row.names(inputExpSet)[expHCncv$vkptInd], size=expHCncv$nvkpt))
  ResNotCommonVar[2] <- list(list(genes=row.names(inputExpSet)[HCncv$vkptInd], size=HCncv$nvkpt))
  ResNotCommonVar[3] <- list(list(genes=row.names(inputExpSet)[FDRncv$vkptInd], size=FDRncv$nvkpt))
  ResNotCommonVar[4] <- list(list(genes=row.names(inputExpSet)[FairnCv$vkptInd], size=FairnCv$nvkpt))
  
  return(list(resCommonVar=ResCommonVar,ResNotCommonVar=ResNotCommonVar))
}

.myDEanalysisFn=function(inputExpSet,labels){
  print("input is supposed to be log transformed data (approach developed on microarray data)")
  #data is supposed to be batch corrected!
...
.myPLSRfilterationFn=function(inputExpSet,labels,testInputData,testLabels,outputsize=NULL){
  ...
  res <- plsr(labels ~ covariates, ncomp = min(max(ncomp,5),dim(dataPrepared$covariates)[2]), data = dataPrepared, validation = "CV",scale=TRUE)
  ...
  #summary(res)
  results=res$fitted.values[1:length(labels),1,1:ncomp]
  colnames(results)=paste0("comp",seq(1:ncol(results)))
  colnames(testInputData)=paste0("comp",seq(1:ncol(results)))
...
.myCPPLSfilterationFn=function(inputExpSet,labels,testInputData,testLabels,outputsize=NULL){
  ...
  res <- cppls(labels ~ covariates, ncomp = min(max(ncomp,5),dim(dataPrepared$covariates)[2]), data = dataPrepared, validation = "CV",segments=5,scale=TRUE)
  ncomp=selectNcomp(res, method = "randomization", plot = F)
  ncomp=max(ncomp,3)
...
.myCvplogisticFilterationFn=function(inputExpSet,labels,outputsize=NULL){
  require(cvplogistic,quietly=T)
  outMCPmmcd=row.names(inputExpSet)[which(cv.cvplogistic(as.numeric(as.factor(labels))-1, (t(.matrixExtraction(inputExpSet))), "mcp", "mmcd",nfold=10)==0)]
  outMCPllacda=row.names(inputExpSet)[which(cv.cvplogistic(as.numeric(as.factor(labels))-1, (t(.matrixExtraction(inputExpSet))), "mcp", "llacda",nfold=10)==0)]
  outMCPadaptive=row.names(inputExpSet)[which(cv.cvplogistic(as.numeric(as.factor(labels))-1, (t(.matrixExtraction(inputExpSet))), "mcp", "adaptive",nfold=10)==0)]
  outSCAD=row.names(inputExpSet)[which(cv.cvplogistic(as.numeric(as.factor(labels))-1, (t(.matrixExtraction(inputExpSet))), "scad",nfold=10)==0)]
  outLasso=row.names(inputExpSet)[which(cv.cvplogistic(as.numeric(as.factor(labels))-1, (t(.matrixExtraction(inputExpSet))), kappa =0)==0)]
  return(list(outMCPmmcd=outMCPmmcd,outMCPllacda=outMCPllacda,outMCPadaptive=outMCPadaptive,outSCAD=outSCAD,outLasso=outLasso))
}

.myCvplogisticFastFilterationFn=function(inputExpSet,labels){
  require(cvplogistic,quietly=T)
  outMCPadaptive=row.names(inputExpSet)[which(cv.hybrid(as.numeric(as.factor(labels))-1, (t(.matrixExtraction(inputExpSet))), "mcp",nfold=5)==0)]
  outSCAD=row.names(inputExpSet)[which(cv.hybrid(as.numeric(as.factor(labels))-1, (t(.matrixExtraction(inputExpSet))), "scad",nfold=5)==0)]
  outLasso=row.names(inputExpSet)[which(cv.cvplogistic(as.numeric(as.factor(labels))-1, (t(.matrixExtraction(inputExpSet))), kappa =0)==0)]
  return(list(outMCPmmcd=outMCPmmcd,outSCAD=outSCAD,outLasso=outLasso))
}

.mySVMfilterationFn=function(inputExpSet,labels){
  require(penalizedSVM,quietly=T)
  labels=(as.numeric(as.factor(labels))-1.5)*2
  
  lambda1.scad <- c (seq(0.01 ,0.05, .01), seq(0.1,0.5, 0.2), 1 )
  svmFitScad<- svm.fs(x=t(.matrixExtraction(inputExpSet)),y=labels, fs.method="scad",
                      ...
                      verbose=FALSE) 
  #svmFitScad=svm.fs(x=t(.matrixExtraction(inputExpSet)),y=labels,lambda1.set=c (seq(0.01 ,0.04, .01),  seq(0.05,1,0.05)),parms.coding = "none",maxIter=10000,inner.val.method = "cv", cross.inner= 10,fs.method="scad", cross.outer= 0)
  resultScad=row.names(inputExpSet)[svmFitScad$model$xind]
  
  svmFitL1=svm.fs(x=t(.matrixExtraction(inputExpSet)),y=labels,lambda1.set=c (seq(0.01 ,0.04, .01),  seq(0.05,1,0.05)),maxIter=10000,parms.coding = "none",inner.val.method = "cv", cross.inner= 10,fs.method="1norm", cross.outer= 0)
  resultL1=row.names(inputExpSet)[svmFitL1$model$xind]
  
  #svmFitElasticNet=svm.fs(x=t(.matrixExtraction(inputExpSet)),y=labels,lambda1.set=c(seq(0.01 ,0.04, .01),seq(0.05,1,0.05)),grid.search = "discrete",parms.coding = "none",maxIter=10000,inner.val.method = "cv", cross.inner= 10,fs.method="DrHSVM", cross.outer= 0)
  #resultElasticNet=row.names(inputExpSet)[svmFitElasticNet$model$xind]
  
  #svmFitScadL2=svm.fs(x=t(.matrixExtraction(inputExpSet)),y=labels,lambda1.set=c (seq(0.01 ,0.04, .01),  seq(0.05,1,0.05)),lambda2.set=c (seq(0.01 ,0.04, .01),grid.search = "discrete",  seq(0.05,1,0.05)),parms.coding = "none",maxIter=10000,inner.val.method = "cv", cross.inner= 10,fs.method="scad+L2", cross.outer= 0)
  #resultScadL2=row.names(inputExpSet)[svmFitScadL2$model$xind]
  
  #return(list(resultScad=resultScad,resultL1=resultL1,resultElasticNet=resultElasticNet,resultScadL2=resultScadL2))
  return(list(resultL1=resultL1,resultScad=resultScad))
}

.myVIFfilterationFn=function(inputExpSet,labels){
  require("VIF",quietly=T)
  indx=vif(as.numeric(as.vector(labels)), t(.matrixExtraction(inputExpSet)), w0 = 0.0005, dw = 0.005, subsize = round(2*(dim(inputExpSet)[1])/3), trace = FALSE);
  return(row.names(inputExpSet)[indx$select])
}

.myFobaFilterationFn=function(inputExpSet,labels){
  require(foba,quietly=T)
  print("not implemented")
  res=data.frame(action=model.foba$path,step=1:lengh(model.foba$path))
}

.myGSEAfilterationFn=function(inputExpSet,labels){
  print("For GSEA analysis, data is supposed to be log2 transformed")
  print("For GSEA analysis, row.names of data is supposed to be entrezIds")
  ds=as.data.frame(2^.matrixExtraction(inputExpSet))
  
  class.vector=as.character(labels)
  
  gs.db ="GeneSetDatabases/msigdb.v5.1.entrez.gmt"   # Gene set database in GMT format
  #gs.db='/Users/gazestani/Documents/bioinfProgram/GSEA-P-R/GeneSetDatabases/selGSs.gmt'
  print(paste("set gneset file location:",gs.db))
  
  doc.string = "GSEAgeneFilteration"
  
  output.directory = ""
  
  resGSEA=.GSEA(input.ds =  ds,input.cls = class.vector,gene.ann = "",gs.db = gs.db,gs.ann = "",output.directory,doc.string,non.interactive.run   = F,
                reshuffling.type = "sample.labels",nperm= 1000,
                weighted.score.type =  1,nom.p.val.threshold   = -1,fwer.p.val.threshold  = -1, fdr.q.val.threshold   = 0.25, topgs                 = 0,
                adjust.FDR.q.val      = F,gs.size.threshold.min = 25,gs.size.threshold.max = 500,reverse.sign = F,preproc.type = 0,random.seed = 111,perm.type= 0,fraction= 1.0,
                replace = F, save.intermediate.results = F,OLD.GSEA  = F,   use.fast.enrichment.routine = T,featureSel=T)
  ...
  if(nrow(x1)>0){
    x1$GS=as.character(x1$GS)
    x1$`FDR q-val`=as.numeric(as.character((x1$`FDR q-val`)))
    
    if(any(x1$`FDR q-val`<(0.25))){
      selGeneSets=x1[x1$`FDR q-val`<(0.25),"GS"]
    }
  }
  if(nrow(x2)>0){
    x2$GS=as.character(x2$GS)
    x2$`FDR q-val`=as.numeric(as.character((x2$`FDR q-val`)))
    if(any(x2$`FDR q-val`<(0.25))){
      
      if(length(selGeneSets)>1){
        selGeneSets=c(x2[x2$`FDR q-val`<(0.25),"GS"],selGeneSets)
      } else {
        if(sum(!is.na(selGeneSets))>0){
          selGeneSets=c(x2[x2$`FDR q-val`<(0.25),"GS"],selGeneSets)
        } else {
          selGeneSets=x2[x2$`FDR q-val`<(0.25),"GS"]
        }
      }
    }
  }
  ...
  if(nrow(x1)>0){
    ...
    if(any(x1$`FDR q-val`<(0.1))){
      selGeneSets2=x1[x1$`FDR q-val`<(0.1),"GS"]
    }
  }
  if(nrow(x2)>0){
    x2$GS=as.character(x2$GS)
    x2$`FDR q-val`=as.numeric(as.character((x2$`FDR q-val`)))
    if(any(x2$`FDR q-val`<(0.1))){
      
      if(length(selGeneSets2)>1){
        selGeneSets2=c(x2[x2$`FDR q-val`<(0.1),"GS"],selGeneSets)
      } else {
        if(sum(!is.na(selGeneSets2))>0){
          selGeneSets2=c(x2[x2$`FDR q-val`<(0.1),"GS"],selGeneSets)
        } else {
          selGeneSets2=x2[x2$`FDR q-val`<(0.1),"GS"]
        }
      }
    }
  }
  ...
  if(sum(!is.na(selGeneSets))>0){
    gsData=.readGeneSetData(gs.db,row.names(inputExpSet),25,500)
    gsData=gsData[row.names(gsData) %in% as.character(selGeneSets),]
    s=apply(gsData,2,function(x){sum(x=="a")})
    indx=s>0
    resFinal=data.frame(geneNames=row.names(inputExpSet)[indx],count=s[indx])
  } else {
    resFinal=NA
  }
  ...
  if(sum(!is.na(selGeneSets2))>0){
    gsData=.readGeneSetData(gs.db,row.names(inputExpSet),25,500)
    gsData=gsData[row.names(gsData) %in% as.character(selGeneSets2),]
    s=apply(gsData,2,function(x){sum(x=="a")})
    indx=s>0
    resFinal2=data.frame(geneNames=row.names(inputExpSet)[indx],count=s[indx])
  } else {
    resFinal2=NA
  }
  return(list(resFinal=resFinal,resFinal2=resFinal2))
}

.myLogisticFwdFn=function(inputExpSet,labels){
  if(dim(inputExpSet)[1]<500){
    inputExpSet=data.frame(cbind(as.numeric(as.factor(labels))-1,t(.matrixExtraction(inputExpSet))))
    colnames(inputExpSet)[1]="labels"
    formulaStr=paste0("labels~",paste(colnames(inputExpSet)[-1],collapse="+"))
    forwards <- step(glm(labels ~ 1,family=binomial,data=inputExpSet),
                     scope=list(lower=as.numeric(as.factor(labels)) ~ 1,upper=formula(formulaStr)), direction="forward",trace=0)
    selIds=forwards$anova$Step[-1]
    indx=regexpr("+ ",selIds)
    selIds=substr(selIds,indx+1,nchar(selIds))
  } else {
    selIds=NA
  }
  return(selIds)
...
.myLmFilterationFn=function(inputExpSet,labels,outputsize=25){
  if(outputsize>50)
    stop("output size above 50 is not supported")
  
  require(leaps,quietly=T)
  trainingInputData=t(.matrixExtraction(inputExpSet))
  ...
  if(length(levels(as.factor(labels)))>2){
    ...
    labels=as.numeric(factor(as.character(labels)))
    labels=labels-1
    dataTrainingPrepared=cbind(labels,trainingInputData)
  }
  colnames(dataTrainingPrepared)[1]="response"
  
  k=5
  folds=sample(1:k,nrow(dataTrainingPrepared),replace=T)
  cv.errors=matrix(NA,k,outputsize,dimnames=list(NULL,past(1:outputsize)))
  for(j in 1:k){
    best.fit=regsubsets(response~.,data=dataTrainingPrepared[folds!=j],nvmax = outputsize)
    for(i in 1:outputsize){
      pred=predict(best.fit,dataTrainingPrepared[folds==j,],id=i)
      cv.errors[j,i]=mean((dataTrainingPrepared$response[folds==j]-pred)^2)
    }
  }
  ...
  reg.best=regsubsets(response~.,data=dataTrainingPrepared,nvmax = outputsize)
  return(coef(reg.best,indx))
}

.myVarImportanceFn=function(inputExpSet,labels,testInputData,testLabels){
  require(caret,quietly=T)
  
  res=filterVarImp(as.data.frame(t(.matrixExtraction(inputExpSet))),factor(labels))
  tmp=res[,1]
  names(tmp)=row.names(res)
  res=tmp
  res=sort(res,decreasing = T)
  inputExpSet=inputExpSet[names(res)[res>summary(res)[5]],]
  testInputData=testInputData[names(res)[res>summary(res)[5]],]
  return(list(features=inputExpSet,testInputData=testInputData))
}

.myCorFilterationFn=function(inputExpSet,outputsize=NULL){
  require(caret,quietly=T)
  corRes=cor(t(.matrixExtraction(inputExpSet)))
  colnames(corRes)=row.names(inputExpSet)
  row.names(corRes)=row.names(inputExpSet)
  x=(findCorrelation(corRes, cutoff = .6, exact = F))
  if(length(x)>0)
    res=row.names(inputExpSet)[-(x)]
  else
    res=row.names(inputExpSet)
  
  return(res)
}

.mySafsFilterationFn=function(inputExpSet,labels){
  require(caret,quietly=T)
  
  ctrl <- safsControl(functions = rfSA,
                      method = "cv",
                      number = 10)
  rf_search <- safs(x = t(.matrixExtraction(inputExpSet)),
                    y = factor(labels),
                    iters = 10,
                    safsControl = ctrl)
...
.myGeneFilteration=function(inputExpSet,labels,testInputData,testLabels,expClassName="proband",method="cov",outputsize=NULL){
  ...
  switch(method,
         ...
         cov=.myCovGeneFilterationFn(inputExpSet = inputExpSet,testInputData = testInputData,outputsize = outputsize),
         var=.myVarGeneFilterationFn(inputExpSet = inputExpSet,testInputData = testInputData,outputsize = outputsize),
         cor=.myCorFilterationFn(inputExpSet = inputExpSet,outputsize = outputsize),
         grn=.myGRNFilterationFn(inputExpSet = inputExpSet,inputLabels = labels,inputExpClassName = expClassName,outputsize = outputsize),
         grn1=.myGRNFilterationFn(inputExpSet = inputExpSet,inputLabels = labels,inputExpClassName = expClassName,outputsize = outputsize,method="grn1"),
         grn3=.myGRNFilterationFn(inputExpSet = inputExpSet,inputLabels = labels,inputExpClassName = expClassName,outputsize = outputsize,method="grn3"),
         grn2=.myGRNFilterationFn(inputExpSet = inputExpSet,inputLabels = labels,inputExpClassName = expClassName,outputsize = outputsize,method="grn2"),
         ...
         logisticFwd=.myLogisticFwdFn(inputExpSet = inputExpSet,labels = labels),
...
.myTMMNormalization=function(inputExpSet){
  require(edgeR,quietly=T)
  myProdFun=function(x,expData,scales){return(expData[,x]*scales[x])}
  
  scales=calcNormFactors(inputExpSet)
  pData(inputExpSet)=cbind(pData(inputExpSet),seqDepth=scales)
  dataN=inputExpSet
  .matrixExtraction(dataN)=sapply(1:dim(inputExpSet)[2],myProdFun,expData=.matrixExtraction(inputExpSet),scales=scales)
...
.myWGCNAfn=function(tmpExpSet,colIds="Ids",diagnosisColName="diagnosis_binary",testInputData=NULL,otherCovariates=NULL,output.directory="resWGCNA/",softPowerValue=NULL,saveModuleGeneList=T,mergeModules=T){
  ...
  data = as.data.frame(.matrixExtraction(tmpExpSet))
  data=cbind(row.names(data),data)
  traitData = pData(tmpExpSet)[,which(colnames(pData(tmpExpSet)) %in% c(colIds,diagnosisColName,otherCovariates))]
  traitData[,diagnosisColName]=as.numeric(as.factor(traitData[,diagnosisColName]))
  #print(dim(tmpExpSet))
  #print(paste0("number of total covariates: ",dim(traitData)[2]-1))
  
  options(stringsAsFactors = FALSE);
  
  datExpr = t(.matrixExtraction(tmpExpSet))
  if(!is.null(testInputData)){
    testInputData=testInputData[row.names(tmpExpSet),]
    testDatExpr = t(.matrixExtraction(testInputData))
  } else {
    testDatExpr=NULL
  }
  datTraits = traitData[rownames(datExpr), -which(colnames(traitData)==colIds)]
  ...
  minModuleSize = 30
  if(is.null(softPowerValue)){
    powers = 1:20
    sft = pickSoftThreshold(datExpr, powerVector = powers, RsquaredCut=0.8, networkType=networkType, blockSize=10000, verbose = 0) # look for other arguments
    softPowerValue = sft$powerEstimate
    #par(mfrow = c(1,2)); 
    cex1 = 0.9
  ...
  adjacency = adjacency(datExpr, type= networkType, power = softPowerValue)
  ...
  MEList = .myModuleEigengenes(datExpr,testExpr = testDatExpr, colors = dynamicColors,softPower = sft$powerEstimate)
  MEList=MEList$ls1
  ...
  nGenes = dim(tmpExpSet)[1]
  nSamples = dim(tmpExpSet)[2]
  
  # module traits relationship
  MEs0 = .myModuleEigengenes(datExpr,testExpr = testDatExpr,colors = dynamicColors,softPower = sft$powerEstimate)
  testMEs0=MEs0$ls2[[1]]
  MEs0 = MEs0$ls1$eigengenes
  
  mergedMEs0 = .myModuleEigengenes(datExpr, testExpr=testDatExpr, colors = moduleColorsMerged,softPower = sft$powerEstimate)
  testMergedMEs0=mergedMEs0$ls2[[1]]
  mergedMEs0 = mergedMEs0$ls1$eigengenes
  
  if(mergeModules){
    MEs = orderMEs(mergedMEs0)
    MEtestList=testMergedMEs0[,colnames(MEs)]
    
  } else {
    MEs = orderMEs(MEs0)
    MEtestList=testMEs0[,colnames(MEs)]
  }
  moduleTraitCor = cor(MEs, datTraits, use = "p")
  #moduleTraitCorDecreasing=moduleTraitCor[order(moduleTraitCor[,1], decreasing=TRUE),]
  ...
  if(mergeModules){
    ...
    geneColors=as.data.frame(cbind(colnames(datExpr),mergedColors))
  } else {
    modColors=unique(dynamicColors, use.name=F)
    geneColors=as.data.frame(cbind(colnames(datExpr),dynamicColors))
  }
  ...
  if(is.null(testInputData)){
    return(list(combinedRes=metaData,eigenGenes=MEs,testEigenGenes=NULL,testLabels=NULL,covariates=datTraits))
  } else {
    return(list(combinedRes=metaData,eigenGenes=MEs,testEigenGenes=MEtestList,testLabels=NULL,covariates=datTraits))
  }
}

.myOriginalWGCNAfn=function(tmpExpSet,colIds="Ids",diagnosisColName="diagnosis_binary",otherCovariates=NULL,output.directory="resWGCNA/",softPowerValue=NULL,saveModuleGeneList=T,mergeModules=T){
  ...
  data=cbind(row.names(data),data)
  traitData = pData(tmpExpSet)[,which(colnames(pData(tmpExpSet)) %in% c(colIds,diagnosisColName,otherCovariates))]
  traitData[,diagnosisColName]=as.numeric(traitData[,diagnosisColName])
  print(dim(tmpExpSet))
  print(paste0("number of total covariates: ",dim(traitData)[2]-1))
  ...
  if(is.null(softPowerValue)){
    powers = 1:20
    sft = pickSoftThreshold(datExpr, powerVector = powers, RsquaredCut=0.8, networkType=networkType, blockSize=10000, verbose = 5) # look for other arguments
    softPowerValue = sft$powerEstimate
    #par(mfrow = c(1,2)); 
    cex1 = 0.9
  ...
  MEList = moduleEigengenes(datExpr, colors = dynamicColors,softPower = sft$powerEstimate)
  MEList=MEList
  MEs = MEList$eigengenes
  MEDiss = 1-cor(MEs);
  METree = flashClust(as.dist(MEDiss), method = "average");
  ...
  # module traits relationship
  MEs0 = moduleEigengenes(datExpr,colors = dynamicColors,softPower = sft$powerEstimate)
  MEs0 = MEs0$eigengenes
  
  mergedMEs0 = moduleEigengenes(datExpr, colors = moduleColorsMerged,softPower = sft$powerEstimate)
  mergedMEs0 = mergedMEs0$eigengenes
  ...
  metaData=cbind(datTraits, MEs)
  return(list(combinedRes=metaData,eigenGenes=MEs,covariates=datTraits))
  
}



.myModuleEigengenes=function (expr,testExpr, colors, impute = TRUE, nPC = 1, align = "along average", 
  ...
  testPrinComps = data.frame(matrix(NA, nrow = dim(testExpr)[[1]], ncol = length(modlevels)))
  ...
  names(testPrinComps) = paste(moduleColor.getMEprefix(), modlevels,sep = "")
  ...
  for (i in c(1:length(modlevels))) {
    ...
    testDatModule = as.matrix((testExpr[, restrict1]))
    n = dim(datModule)[1]
    p = dim(datModule)[2]
    pc = try({
      ...
      svd1 = prcomp(datModule, scale.=T)
      
      svdTest = predict(svd1,testDatModule)
      
      veMat = cor(svd1$x[, c(1:min(n, p, nVarExplained))], 
                  datModule, use = "p")
      varExpl[c(1:min(n, p, nVarExplained)), i] = rowMeans(veMat^2, 
                                                           na.rm = TRUE)
      list(trainingPC=svd1$x[, 1],testPC=svdTest[,1])
      
    }, silent = TRUE)
    if (class(pc) == "try-error") {
      print("error in PCA analysis")
      ...
      if (subHubs) {
        ...
        pc = try({
          ...
          testScaledExpr = scale(t(testDatModule))
          ...
          testPcxMat = testScaledExpr * matrix(kIM * alignSign, 
                                               nrow = nrow(scaledExpr), ncol = ncol(scaledExpr), 
                                               byrow = TRUE)/sum(kIM)
          testPcx = rowMeans(testPcxMat, na.rm = TRUE)
          
          
          varExpl[1, i] = mean(cor(pcx, t(datModule), 
                                   use = "p")^2, na.rm = TRUE)
          
          list(trainingPC=pcx,testPC=testPcx)
    ...
    if (class(pc) == "try-error") {
      ...
      PrinComps[, i] = pc$trainingPC
      testPrinComps[, i] = pc$testPC
      ae = try({
        if (isPC[i]) 
          scaledExpr = scale((datModule))
        ...
        if (align == "along average") {
          ...
          if (corAve < 0) {
            ...
            testPrinComps[, i] = -testPrinComps[, i]
          }
        }
        0
      }, silent = TRUE)
  ...
  if (returnValidOnly && sum(!validMEs) > 0) {
    ...
    testPrinComps = testPrinComps[, validMEs]
  ...
  ls1=list(eigengenes = PrinComps,testEigengenes = testPrinComps, averageExpr = averExpr, varExplained = varExpl, 
  ...
  ls2=list(testEigengenes = testPrinComps)
  return(list(ls1=ls1,ls2=ls2))
}
...
.myFDRfn=function(pval,numberOfTests){
  p=pval
...
print(".myWGCNAfn(tmpExpSet,colIds='Ids',diagnosisColName='diagnosis_binary',otherCovariates=c('age'),output.directory='Desktop/resWGCNA/',softPowerValue=NULL,saveModuleGeneList=T)")
print("*************")
print("RNAseq analysis")
print(".myTMMNormalization(inputExpSet)")
print(".myRNAseqLowFilter(inputExpSet,labels,minCount=3,minPercentage=0.7)")
print(".myRNAseqHighFilter(inputExpSet,count=20)")
print("*************")
print("Gene filteration methods")
print(".myGeneFilteration(inputExpSet,labels,method='cov',outputsize=NULL)")
print("available methods: cov, var, cor, WGCNA, grn, grn1, grn3, zscore, SIS, lm, selectV, PLSR, CPPLS, PCR, cvplogistic, cvplogisticFast, svm, vif, logisticFwd, foba, varImportance, Safs")
...
.GSEA.ReadClsVector <- function(class.list) {
  #
  # Input is a class vector CLS file and defines phenotype and class labels vectors for the samples in a gene expression file (RES or GCT format)
...
.GSEA= function(
  ...
  if (output.directory != "")  {
    ...
    if (is.list(input.cls)||is.vector(input.cls)) {
  ...
  if(is.list(input.cls)) {
    ...
    if(is.vector(input.cls))
    {
      CLS <- .GSEA.ReadClsVector(input.cls)
    } else {
  ...
  if(featureSel==F){
    ...
    print(paste0("Number of genes:", N,"; ","Number of Gene Sets:", Ng,"; ","Number of samples:", Ns))
  }
  ...
  #print(paste("Computing observed enrichment for gene sets"));
  ...
  #print("Computing random permutations' enrichment for gene sets");
  ...
  if(featureSel==F){
    ...
    glob.p.vals.sorted=rep(2,length(FDR.median.sorted))
  }
  ...
  if (Ng > 1&featureSel==F) { # make these plots only if there are multiple gene sets.
